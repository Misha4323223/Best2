...
✅ 28 июня 2025: ИСПРАВЛЕНИЕ ГЕНЕРАЦИИ ИЗОБРАЖЕНИЙ
  • Исправлена критическая ошибка в smart-router.js
  • Проблема: ИИ анализ запросов на генерацию возвращал текстовые ответы вместо команды "ГЕНЕРАЦИЯ_ИЗОБРАЖЕНИЯ"
  • Решение: Добавлена прямая проверка ключевых слов перед обращением к ИИ
  • Новая логика: сначала проверяем ключевые слова ('создай принт', 'техносамурай', 'нарисуй' и т.д.)
  • Если найдены - сразу переходим к генерации без ИИ анализа
  • Удален дублирующий код генерации изображений
  • Исправлено дублирование переменных (imageKeywords -> directImageKeywords)
  • Теперь запросы типа "Создание принта техносамурая" должны корректно генерировать изображения
